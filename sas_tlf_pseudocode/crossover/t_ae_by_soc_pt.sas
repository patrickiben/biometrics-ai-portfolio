...
* NOTE      : PSEUDOCODE. Crossover -> events attributed to TRTA at onset;
*             columns = each analysis treatment + Total. Counts = PARTICIPANTS with
*             >=1 event (distinct USUBJID), NOT event rows. n (%) per treatment;
*             % denominator = treatment-exposed N (from %bign on ADEX).
*             SOC sorted by overall frequency desc; PT within SOC desc.
*             Within-participant design: a participant may be counted under >1 column.
******************************************************************************/
%include "../00_setup_macros.sas";
%setup(study=CP-101, adam=/data/adam, out=/data/tfl);
%designvars(design=CROSSOVER);            /* TRTVAR=TRTA  TRTNVAR=TRTAN       */

/* per-treatment exposed denominators (crossover: by TRTA, not ADSL arm)      */
proc sql;
...
%tfltitle(num=14.3.1.2, type=Table,
   ...
   foot=%str(Events attributed to the analysis treatment (TRTA) at onset. A participant is counted once at each level per treatment; within-participant crossover -> a participant may appear under more than one treatment. MedDRA v27.0. %% = participants with the event / treatment-exposed N.));
proc report data=_wide nowd split='|';
  columns socn ptn level term ("Analysis Treatment" /* TRTAN cols + Total */);
