/******************************************************************************
* TABLE     : t_lab_shift  (MAD - Multiple Ascending Dose)
* TITLE     : Shift from Baseline to Worst Post-Baseline Normal Range
*             Category by Laboratory Parameter
* POPULATION: Safety Population (SAFFL='Y')
* INPUT     : ADLB (PARAM/PARAMCD, BNRIND, ANRIND, AVISITN, TRT01A/TRT01AN)
* NOTE      : PSEUDOCODE. Shift = Day 1 pre-dose baseline normal-range indicator
*             (BNRIND) vs WORST post-baseline indicator (ANRIND), cross-
*             tabulated. Counts = PARTICIPANTS (distinct USUBJID). % denominator =
*             participants with a non-missing baseline category in that dose level.
*             MAD: parallel cohorts, one (dose) treatment per participant -> one
*             panel per dose level (TRT01A). Repeated dosing -> the worst post-
*             baseline category is the most extreme over the ENTIRE multiple-
*             dose period (all on-treatment visits Day 1..Day N), so a category
*             worsening that accumulates with repeated dosing is captured.
******************************************************************************/
...
/*--- worst post-baseline category per participant/dose/parameter --------------*
* MAD: CLASS carries &TRTVAR (=TRT01A) so the worst category is taken over    *
* the whole multiple-dose period for the participant's single dose-level cohort.  *
...
%tfltitle(num=14.3.4.2, type=Table,
   ...
   foot=%str(Rows = Day 1 pre-dose baseline category (BNRIND); columns = worst post-baseline category over the multiple-dose period. Counts = participants with a non-missing baseline category. % = n / participants with baseline category in dose level. One panel per dose-level cohort (MAD; one dose per participant).));
