...
*             Min-Max. Categorical: n (%). MAD: parallel ascending-dose cohorts,
*             one dose level per participant -> columns = TRT01A/TRT01AN (= dose
*             level, placebo pooled) + Total, ordered ascending dose; % denom =
*             SAFFL N per dose column. Baseline values feed CHG/PCHG and
*             dose-proportionality/accumulation summaries downstream.
******************************************************************************/
...
%tfltitle(num=14.1.3, type=Table, text=Baseline Disease and Participant Characteristics,
          ...
          foot=%str(Column = assigned dose level (TRT01A); placebo pooled, columns ordered by ascending dose. Percentages based on the number of participants in the Safety Population per dose level. Baseline characteristics from ADSL.));
