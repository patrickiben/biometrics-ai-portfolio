/******************************************************************************
* TABLE     : t_ada_summary  (MAD - Multiple Ascending Dose)
...
*             baseline vs post-baseline ADA over the repeated-dosing period)
...
*             TRT01A/TRT01AN (= dose level, placebo pooled). Repeated dosing
*             gives multiple post-baseline ADA sampling visits across the
*             treatment period; the ADaM-derived participant-level flags collapse
*             that sampling history into the displayed categories. Counts =
*             PARTICIPANTS (distinct USUBJID), NOT records; % denominator =
*             ADA-evaluable N per column (from %bign). Categories: baseline-
...
/*--- header denominators: ADA-evaluable N per dose column, FROM ADIS ---------
* One authoritative ADA-evaluable denominator source = ADIS (ADAFL='Y'), distinct
* USUBJID (matches the R twin), NOT one-row-per-participant ADSL.               */
proc sql;
...
* ADIS carries the derived immunogenicity flags computed over all repeated-
* dosing ADA samples; select the participant-level analysis records (e.g. PARAMCD
* for overall ADA status / NAb status).                                      */
data ada;
...
%tfltitle(num=14.5.1.1, type=Table,
   ...
   foot=%str(Counts = participants (distinct USUBJID); % = participants / ADA-evaluable N per dose level (denominator from ADIS ADAFL='Y', distinct USUBJID). Participant-level ADA flags derived over all repeated-dosing samples. Treatment-emergent = induced or boosted. NAb = neutralizing antibody (among ADA-positive). MAD: placebo pooled, columns ordered by ascending dose.));
