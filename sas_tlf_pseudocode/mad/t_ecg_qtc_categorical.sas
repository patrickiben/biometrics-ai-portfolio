...
*             Analysis by Dose Level
* POPULATION: Safety Population (SAFFL='Y'), on-treatment
* INPUT     : ADEG  (PARAMCD='QTCF'; AVAL, CHG, AVISITN, ADY; QTcF category
*             flags from ADaM where available)
* NOTE      : PSEUDOCODE. MAD design: column = dose level (TRT01A/TRT01AN;
*             placebo pooled in ADaM). Counts = PARTICIPANTS (distinct USUBJID)
*             meeting each threshold at ANY post-baseline timepoint over the
*             whole multiple-dose period (worst-case across all dosing days);
*             % denominator = N per dose column (%bign). Repeated dosing means
*             the worst-case scan spans every study day, so cumulative QTcF
*             effects emerging on later dosing days are captured.
*             Standard QTcF outlier categories: absolute value (>450, >480,
*             >500 ms) and change from baseline (>30, >60 ms).
******************************************************************************/
...
/*--- on-treatment post-baseline QTcF records over the MAD period -----------*/
data qtcf;
  set adam.adeg(where=(SAFFL='Y' and ANL01FL='Y' and AVISITN>0
                       and PARAMCD='QTCF'));
run;

/*--- per-participant worst-case flags (any post-baseline timepoint, any dosing
* day over the multiple-dose period meets cutoff). Prefer ADaM-derived
* category flags; thresholds shown for transparency.                          */
proc sql;
...
%tfltitle(num=14.3.6.2, type=Table,
   text=%str(QTcF Interval and Change from Baseline - Categorical Outlier Analysis by Dose Level),
   pop=Safety Population,
   foot=%str(Counts = participants (distinct USUBJID) meeting the criterion at any post-baseline timepoint over the multiple-dose period (worst-case across dosing days). Percentages based on N per dose level in the Safety Population (placebo pooled). QTcF = Fridericia-corrected (ADaM).));
proc report data=_wide nowd split='|';
  columns ord catlbl ("Dose Level" _NAME_ /* ascending dose cols + Total */);
