/******************************************************************************
* TABLE     : t_medical_history  (Multiple Ascending Dose)
* TITLE     : Medical History
* POPULATION: Safety Population (SAFFL='Y')
* INPUT     : ADMH
* NOTE      : PSEUDOCODE. Counts = PARTICIPANTS with >=1 condition (distinct
*             USUBJID), NOT history rows. n (%) per dose level; % denom = SAFFL
*             N per dose column. Summarized by MedDRA SOC (MHBODSYS) and
*             Preferred Term (MHDECOD). MAD: parallel ascending-dose cohorts,
*             one dose level per participant -> columns = TRT01A/TRT01AN (= dose
*             level, placebo pooled). Medical history is baseline/static, so
*             layout matches parallel; only the column variable is re-pointed
*             to the dose-level cohorts.
******************************************************************************/
...
/* Safety-population medical-history records; keep PRESENT conditions only
   (MHOCCUR='Y', or missing where condition-list collection has no Y/N), matching
   the R twin. Treatment + SOC/PT carried on ADMH (no re-derivation).          */
data admh; set adam.admh(where=(SAFFL='Y' and (upcase(MHOCCUR)='Y' or missing(MHOCCUR)))); run;
...
%tfltitle(num=14.1.7, type=Table, text=Medical History,
          ...
          foot=%str(Column = assigned dose level (TRT01A); placebo pooled, columns ordered by ascending dose. A participant is counted once at each level. MedDRA v27.0. Percentages based on Safety Population N per dose level.));
