/******************************************************************************
* TABLE     : t_ada_summary  (Parallel-group / per-dose)
...
*             variable = TRT01A/TRT01AN (= dose). Counts = PARTICIPANTS (distinct
*             USUBJID), NOT records; % denominator = ADA-evaluable N per
*             column (from %bign). Categories: baseline-positive, treatment-
*             emergent (induced + boosted), persistent/transient, NAb-positive.
******************************************************************************/
...
/*--- participant counts per ADA category x treatment (distinct USUBJID) ------
...
%tfltitle(num=14.5.1.1, type=Table,
   ...
   foot=%str(Counts = participants (distinct USUBJID); % = participants / ADA-evaluable N per treatment (dose). Treatment-emergent = induced or boosted. NAb = neutralizing antibody (among ADA-positive). Parallel-group.));
proc report data=_wide nowd split='|';
  columns cat ("Treatment (Dose)" /* dose cols incl. Total */);
