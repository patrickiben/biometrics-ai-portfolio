...
*             Counts = PARTICIPANTS (distinct USUBJID) meeting each threshold at
*             any post-baseline timepoint; % denominator = N per arm (%bign).
...
/*--- count PARTICIPANTS meeting each category per arm (distinct USUBJID) --------*/
...
proc transpose data=_disp out=_wide;
  by ord catlbl;  id trtn;  var value;     /* one col per arm + Total        */
run;

%tfltitle(num=14.3.5.2, type=Table,
   text=%str(QTcF Interval and Change from Baseline - Categorical Outlier Analysis by Treatment),
   pop=Safety Population,
   foot=%str(Counts = participants (distinct USUBJID) meeting the criterion at any post-baseline timepoint. Percentages based on N per treatment arm in the Safety Population. QTcF = Fridericia-corrected (ADaM).));
proc report data=_wide nowd split='|';
  columns ord catlbl ("Treatment Arm" _NAME_ /* arm cols + Total */);
