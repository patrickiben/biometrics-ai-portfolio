/******************************************************************************
* TABLE     : t_ae_by_severity  (Parallel-group)
...
*             n (%) per arm x severity; % denominator = SAFFL N per arm.
...
/*--- participant counts per SOC*PT x arm x worst-severity -------------------------*/
proc sql;
...
/*--- "Any TEAE" overall row: participant once at overall worst severity, per arm --*/
proc means data=adae nway noprint;
...
run;
/* columns = arm x severity (Mild/Moderate/Severe + arm Total); participants unique */
proc sort data=_rep; by socn ptn level term trtn sevcat; run;
proc transpose data=_rep out=_wide;
  by socn ptn level term;
  id trtn sevcat;                              /* one block per arm x severity   */
  var value;
run;

%tfltitle(num=14.3.1.2, type=Table,
   text=%str(Treatment-Emergent Adverse Events by System Organ Class, Preferred Term and Maximum Severity),
   pop=Safety Population,
   foot=%str(A participant is counted once at each level at their maximum severity. Severity per AESEVN (Mild/Moderate/Severe). % = participants / N in arm. MedDRA v27.0.));
proc report data=_wide nowd split='|';
  columns socn ptn level term ("Treatment (Dose) by Maximum Severity" /* arm x sev cols */);
