...
* NOTE      : PSEUDOCODE. Report arithmetic n, Mean, SD, CV%, Geo Mean,
*             Geo CV%, Median, Min, Max. Tmax = Median (Min, Max) ONLY.
*             Geometric stats undefined if AVAL<=0 (exclude / footnote).
******************************************************************************/
...
%tfltitle(num=14.4.1.1, type=Table, text=Summary of Plasma Pharmacokinetic Parameters,
          pop=Pharmacokinetic Parameter Population,
          foot=%str(CV% arithmetic; Geo CV% = 100*sqrt(exp(s^2_log)-1). Tmax: Median (Min, Max). N excludes BLQ-driven non-estimable parameters.));
proc report data=_wide nowd split='|';
  columns PARAM PARAMCD stat ("Treatment (Dose)" /* dose cols */);
