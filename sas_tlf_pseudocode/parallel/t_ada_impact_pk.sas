/******************************************************************************
* TABLE     : t_ada_impact_pk  (Parallel-group / per-dose)
...
*             level ADA status from ADIS; ADSL for treatment column
* NOTE      : PSEUDOCODE. Parallel-group: column = TRT01A/TRT01AN (= dose);
*             within each dose, exposure PK parameters summarized by ADA
...
%tfltitle(num=14.5.2.1, type=Table,
   ...
   foot=%str(Exposure parameters summarized by ADA status (positive/negative) within each treatment (dose). CV% arithmetic; Geo CV% = 100*sqrt(exp(s^2_log)-1). Tmax: Median (Min, Max). Descriptive subgroup comparison; parallel-group.));
proc report data=_wide nowd split='|';
  columns PARAM PARAMCD stat ("Treatment (Dose) by ADA Status" /* dose x ADA cols */);
