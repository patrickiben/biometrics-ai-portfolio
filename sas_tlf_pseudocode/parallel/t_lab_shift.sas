...
* TITLE     : Shift from Baseline to Worst Post-Baseline NCI/Normal Range
...
*             with a non-missing baseline category in that arm. One table block
*             per parameter; separate page panel per treatment arm.
*             Parallel design: column/panel = TRT01A.
******************************************************************************/
...
* HIGH. Carry signed extreme so the worst-tied direction is recoverable.      */
data lb;
  set lb;
...
/*--- denominator: participants with non-missing baseline category, per arm -----*/
proc sql;
...
quit;
/* merge Nbase by arm+param -> pct = nsubj/Nbase*100 ; value = "n (xx.x%)"   */
...
%tfltitle(num=14.3.4.2, type=Table,
   ...
   foot=%str(Rows = baseline category (BNRIND); columns = worst post-baseline category. Counts = participants with a non-missing baseline category. % = n / participants with baseline category in arm. One panel per treatment arm.));
proc report data=_wide nowd split='|';
  ...
  define &TRTNVAR / order 'Treatment Arm' width=18;   /* panel per arm        */
