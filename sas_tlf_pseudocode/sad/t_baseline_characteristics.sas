/******************************************************************************
* TABLE     : t_baseline_characteristics  (Single Ascending Dose)
...
*             Min-Max. Categorical: n (%). SAD: parallel ascending-dose
*             cohorts, ONE treatment per participant; columns = TRT01A = assigned
*             dose level (placebo often pooled) + Total; % denom = SAFFL N.
******************************************************************************/
...
* In SAD, baseline renal/hepatic/cardiac status supports dose-escalation
* safety review at each ascending cohort.                                     */
...
* category), CYPPHENO (CYP phenotype/genotype group), SMOKSTAT.              */
...
proc transpose data=_all out=_wide;
  by ord charlbl stat; id &TRTNVAR; var value;     /* one col per dose + Total */
run;

%tfltitle(num=14.1.3, type=Table, text=Baseline Disease and Participant Characteristics,
          pop=Safety Population,
          foot=%str(Column = assigned dose level (TRT01A); placebo may be pooled across cohorts. Percentages based on the number of participants in the Safety Population per dose level. Baseline characteristics from ADSL.));
