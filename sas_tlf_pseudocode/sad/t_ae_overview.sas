...
*             NOT event rows. n (%) per dose level; % denominator = SAFFL N
*             per dose column.
*             SAD design: parallel ascending-dose cohorts -> column var =
*             TRT01A/TRT01AN (= dose level). Single dose only, so NO accumulation
*             concept here. Placebo is typically POOLED across cohorts into one
*             column (controlled via ADaM TRT01A/TRT01AN bookkeeping; no
*             re-derivation in TLF code).
******************************************************************************/
...
%tfltitle(num=14.3.1, type=Table,
   ...
   foot=%str(TEAE = treatment-emergent AE (TRTEMFL='Y'). A participant is counted once in each category. Columns = ascending dose levels (placebo pooled). % = participants with the event / N in dose column. MedDRA v27.0.));
