...
* TITLE     : Summary of ECG Parameters and Change from Baseline by Dose
*             and Visit
...
*             participant; column = dose level (TRT01A), placebo pooled per ADaM.
*             Descriptive only (n, Mean(SD), Median, Min, Max) for AVAL and
*             CHG at each scheduled visit. QTcF (Fridericia) is the primary
*             corrected interval; dose ordering supports a dose-trend read.
******************************************************************************/
...
%tfltitle(num=14.3.5.1, type=Table,
   text=%str(Summary of ECG Parameters and Change from Baseline by Dose and Visit),
   pop=Safety Population,
   foot=%str(QTcF = Fridericia-corrected QT (ADaM). Change from baseline = post-baseline value - baseline (ADaM CHG). Descriptive statistics only; single dose per participant across ascending dose cohorts (placebo pooled per ADaM).));
