...
* TITLE     : Summary of Vital Signs and Change from Baseline by Dose and Visit
* POPULATION: Safety Population (SAFFL='Y')
* INPUT     : ADVS  (PARAMCD: SYSBP, DIABP, PULSE, TEMP, RESP; AVAL, CHG,
*             BASE, AVISIT/AVISITN, BNRIND/ANRIND)
* NOTE      : PSEUDOCODE. SAD: parallel dose cohorts, one (single) dose per
*             participant; column = dose level (TRT01A), placebo typically pooled
*             across cohorts via ADaM. Descriptive only (n, Mean(SD), Median,
*             Min, Max) for AVAL and CHG at each scheduled visit. Single dose,
*             so no accumulation / no by-period structure.
******************************************************************************/
%include "../00_setup_macros.sas";
%setup(study=CP-101, adam=/data/adam, out=/data/tfl);
%designvars(design=SAD);               /* -> TRTVAR=TRT01A, TRTNVAR=TRT01AN  */

/* column denominators (N=) per dose level + Total (placebo pooled in ADaM)  */
...
/*--- one block per parameter: AVAL then CHG, by dose level x visit ---------
* descstat classes on &TRTVAR &TRTNVAR (column = dose) and AVISIT/AVISITN
* (rows); dp follows the ADaM decimal hint for each vital.                    */
...
proc transpose data=_all out=_wide;
  ...
  id &TRTNVAR;  var value;             /* one col per dose level + Total      */
run;

%tfltitle(num=14.3.7.1, type=Table,
   text=%str(Summary of Vital Signs and Change from Baseline by Dose and Visit),
   pop=Safety Population,
   foot=%str(Change from baseline = post-baseline value - baseline (ADaM CHG). Descriptive statistics only; single dose per participant across ascending dose cohorts (placebo pooled per ADaM).));
proc report data=_wide nowd split='|';
  columns ord param measure AVISITN AVISIT stat
          ("Dose Level" _NAME_ /* dose cols + Total */);
