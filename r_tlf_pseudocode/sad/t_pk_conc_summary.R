################################################################################
# TABLE     : t_pk_conc_summary  (SAD - Single Ascending Dose / per-cohort)
# TITLE     : Summary of Plasma Concentrations by Dose Cohort, Nominal Time
...
#             per dose cohort x nominal time. Geometric stats on the LOG scale
#             (pkstats). BLQ handled per SOP (count + rule), excluded from geo
#             stats (x > 0). SAD = parallel cohorts: column = TRT01A = dose level
#             (one treatment / participant, single dose -> no accumulation). Placebo
#             often pooled across cohorts. Descriptive only -- no inferential test.
################################################################################
...
by <- c(dv$trtvar, "PARAM", "ATPTN", "ATPT")            # dose cohort x analyte x nominal time
...
## --- stat rows x dose-cohort cols, blocked by analyte then nominal time -----
tab <- cont %>%
...
ttl <- tfl_titles(num = "14.4.1.2", type = "Table",
   text = "Summary of Plasma Concentrations by Dose Cohort and Nominal Sampling Time",
   ...
   foot = paste("Geo CV% = 100*sqrt(exp(s^2_log)-1); geometric stats on log scale,",
                ...
                "actual times used for parameter derivation. Column = dose cohort (single dose).",
                "Placebo pooled across cohorts per SAP."))
