...
#             (NOT med rows). % denominator = Safety N per arm from bign().
#             Columns = treatment arms (TRT01A) + Total.
################################################################################
...
## counts DISTINCT participants per cell; % vs Safety N per arm + Total. Order
...
cm_section <- function(df, section_label) {
  ...
  rows <- purrr::pmap_dfr(cls_order, function(CMCLAS, nn, cord) {
    ...
    hdr_per <- sub %>% group_by(trt = .data[[dv$trtvar]]) %>%
      summarise(n = n_distinct(USUBJID), .groups = "drop")
    ...
    pts <- purrr::pmap_dfr(pt_order, function(CMDECOD, nn, pord) {
      ...
      p  <- s2 %>% group_by(trt = .data[[dv$trtvar]]) %>%
        summarise(n = n_distinct(USUBJID), .groups = "drop")
...
## --- stack sections (rows) x treatment columns, render ---------------------
...
ttl <- tfl_titles(num = "14.1.6", type = "Table",
   ...
   foot = "Participants counted once per class / medication (distinct USUBJID). Percentages based on Safety Population N per arm. WHODrug Global Bxx coding; ATC class = CMCLAS.")
