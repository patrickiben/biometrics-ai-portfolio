################################################################################
# TABLE     : t_lab_shift  (Multiple Ascending Dose)
...
#             variable = DOSE LEVEL (dv$trtvar = TRT01A), placebo pooled. WORST is
#             taken over ALL on-treatment readings (every dosing day through end of
#             treatment), so cumulative/repeat-dose effects are captured. Cross-tab
#             Baseline category (BNRIND) x WORST post-baseline category per participant/
#             parameter. Counts = distinct PARTICIPANTS; % denominator = participants with a
#             non-missing baseline in that row, per dose level. WORST = category
#             FURTHEST from NORMAL (not the max() of the LOW<NORMAL<HIGH ordinal);
#             ties resolved to High.
################################################################################
...
## post-baseline records with a usable normal-range index; ON-treatment spans
## the whole repeated-dosing period, so "worst" reflects cumulative exposure
post <- adam$adlb %>%
...
## per participant/param keep the row with max dist (across ALL dosing days);
## on ties prefer HIGH over LOW
worst <- post %>%
...
ttl <- tfl_titles(num = "14.3.4.2", type = "Table",
   ...
   foot = paste("Worst post-baseline category = result furthest from normal",
                "(ties resolved to High), taken over all on-treatment dosing days",
                "(MAD repeated dosing). A participant counted once per parameter.",
