...
function parse(md) {
  ...
  while (i < lines.length) {
    ...
    // fenced code block ``` ... ```  -> monospaced, shaded, indentation preserved
    if (/^\s*```/.test(line)) {
      i++;
      const code = [];
      ...
      code.forEach((c, ci) => {
        out.push(new Paragraph({
          shading: { fill: "F2F1FB", type: ShadingType.CLEAR },
          spacing: { before: ci === 0 ? 80 : 0, after: ci === code.length - 1 ? 80 : 0, line: 230 },
          border: { left: { style: BorderStyle.SINGLE, size: 14, color: "6D28D9", space: 8 } },
          children: [new TextRun({ text: (c.replace(/\t/g, "    ")) || " ", font: "Consolas", size: 16, color: "241A3D" })],
        }));
      });
      continue;
    }
